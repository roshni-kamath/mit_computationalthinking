...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	accumulate(T, init=sir_0) do prev, a
		euler_SIR_step(β, γ, prev, a)
	end
end
...
We have a group of susceptible individuals who can get sick with a transmission rate Beta, defined as the chance of contact with an infected individual * the probability of disease transmission. Once infected, individuals can recover with a recovery rate of Gamma, given by the inverse of the duration of the illness.
...
@bind β Slider(0.00:0.0001:0.2; default = 0.05, show_value=true)

# ╔═╡ d6f6b102-43b7-11eb-05cf-f1638ff942fb
@bind γ Slider(0.00:0.0001:0.2; default = 0.05, show_value=true)
...
function gradient_descent_1d_step(f, x0; η=0.01)
	...
	x0 - η*finite_difference_slope(f, x0)
end
...
When the difference is small between the latest and penultimate answer.
"""
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	x_dir, y_dir =	gradient(f, x0, y0)
	return [x0 - η*x_dir, y0 - η*y_dir]
end
...
function loss_dice(μ, σ)
	...
	return sum(gauss.(dice_x, μ, σ) - dice_y)
end
...
found_μ, found_σ = let
	...
	# your code here
	gradient_descent_2d(loss_dice, 30, 1, η=0.5)
...
function loss_sir(β, γ)
	results = euler_SIR_step(β, γ, [0.99, 0.01, 0.00], length(hw4_T))
	loss = 0
	
	for i in 1:length(hw4_results)
		loss += first(sum(abs.(hw4_results[i]-results[i])))
	end
	loss
end
