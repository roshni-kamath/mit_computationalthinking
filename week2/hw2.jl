...
Seems to have avoided about 1/3 of the allocations. Not sure why... but I guess because there 3 to be made: 1) part of row left of the pixel removed, 2) the part of the row to the right, 3) the `vcat` of those 2.
...
function greedy_seam(energies, starting_pixel::Int)
	...
	j_seam = Array{Int, 1}(undef, m)  # we will return this
	j_seam[1] = starting_pixel
	for i in 2:m
		# findmin returns (minval, index)
		j_try = clamp.([-1, 0, 1] .+ j_seam[i-1], 1, n)
		_, ind = findmin(energies[i, j_try])
		j_seam[i] = clamp(ind - 2 + j_seam[i-1], 1, n)
		# -2 takes j in 1,2,3 (found by findmin) to j in -1,0,1 space (rt. j_seam[i-1])
		#
		# this would be easier to follow with one of those fancy indices
		# CartesianIndices?
	end
	return j_seam
end
...
function least_energy(energies, i, j)
	...
	# base case
	if i == m
	   return energies[i, j], missing # no need for recursive computation in the base case!
	end
	#
	# induction
	...
	res = [least_energy(energies, i+1, j_) for j_ in j_try]
	
	# return the result with minimum energy
	res_emin, ires_emin = findmin([r[1] for r in res])
	return res_emin + energies[i,j], j_try[ires_emin]	
	return 
end
...
function recursive_seam(energies, starting_pixel)
	...
	for i = 2:m
		j = least_energy(energies, i-1, j_seam[i-1])[2]
		j_seam[i] = j
	end
	return j_seam
end
